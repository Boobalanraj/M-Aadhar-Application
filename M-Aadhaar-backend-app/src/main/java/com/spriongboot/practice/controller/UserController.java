...
import com.spriongboot.practice.service.UserService;
...
public class UserController {
	...
	public ResponseEntity<UserEntity> adduser(@RequestBody UserEntity u){
		...
		UserEntity user= service.addUser(u);
		
		if(user!=null)
			return new ResponseEntity<UserEntity>(user,HttpStatus.CREATED);
		else
			return new ResponseEntity<UserEntity>(user,HttpStatus.INTERNAL_SERVER_ERROR);
	...
	@PutMapping("/{id}")
	public ResponseEntity<Object> updateUser(@PathVariable int id,@RequestBody UserEntity newUser){
